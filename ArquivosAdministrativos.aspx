    ...
    <div class="container" data-ng-app="ArquivosAdministrativos" data-ng-controller="ArquivosAdministrativosController">
      <form name="mainForm" data-ng-class="{display: display}">
        ...
        <div class="row header">
          ...
          <div class="logo"><a href="../" title="Helbor"><img alt="Helbor" src="/sites/IntranetHelbor/SiteAssets/images/helbor-logo.png" /></a></div>
        </div>
        ...
        <div class="row">
          <div class="col-md-4">
            <div class="form-group" data-ng-class="{ 'has-error': sent && hasError('item.Colaborador') }" data-ng-if="canView('item.Colaborador')">
              ...
              <div class="people-picker" data-ng-class="{ 'disabled': display || !canEdit('item.Colaborador') }" data-toggle="tooltip" title="Este campo é responsável por armazenar o nome do colaborador que irá fazer o upload de arquivo.">
                <input type="text" class="form-control" data-field="item.Colaborador" data-ng-model="item.ColaboradorText" data-ng-keyup="searchPeople($event, 'item', 'Colaborador', item)" data-ng-if="!display && canEdit('item.Colaborador')" data-ng-hide="item.Colaborador" />
                <ul class="people-picker-choices" data-ng-show="item.ColaboradorChoices.length">
                  <li data-ng-repeat="p in item.ColaboradorChoices" data-ng-click="selectPerson('item.Colaborador', item, p, null)">{{p.Title}}</li>
                </ul>
              </div>
              <div class="field-values">
                <div class="field-value single" data-ng-show="item.Colaborador">{{item.Colaborador.Title}}<!--<div class="remove-item" data-ng-if="!display && canEdit('item.Colaborador')" data-ng-click="item.Colaborador = null"></div>--></div>
              </div>
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group" data-ng-class="{ 'has-error': sent && hasError('item.Departamento') }" data-ng-if="canView('item.Departamento')">
              ...
              <select name="item.Departamento" class="form-control" selectpicker="{ title: '', liveSearch: true, liveSearchNormalize: true }" data-ng-model="item.Departamento" data-ng-disabled="display || !canEdit('item.Departamento') || bloqueiaDepartamento" data-ng-options="c.Id as c.Title for c in item_DepartamentoChoices" data-toggle="tooltip" title="Este campo é responsável por armazenar o nome do departamento correspondente do colaborador." data-ng-required="!display && canEdit('item.Departamento')"></select>
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group" data-ng-class="{ 'has-error': sent && hasError('item.Data') }" data-ng-if="canView('item.Data')">
              ...
              <input type="text" name="item.Data" class="form-control date-picker" ui-date="datePickerOptions['item.Data']" ui-date-format="dd/mm/yy" data-ng-model="item.Data" data-ng-disabled="display || !canEdit('item.Data') || bloqueiaData" data-toggle="tooltip" title="Este campo é responsável por armazenar a data que irá ser o upload de arquivo." readonly data-ng-required="!display && canEdit('item.Data')" />
            </div>
          </div>
        </div>
        ...
        <div class="row controls">
          <div class="col-xs-12">
            ...
            <button type="button" class="btn btn-danger cancel-button" data-ng-click="cancel()" data-ng-hide="display || !canSend()">Cancelar</button>
            <button type="button" class="btn btn-default close-button" data-ng-click="cancel()" data-ng-show="display || !canSend()">Fechar</button>
